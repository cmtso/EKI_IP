...
axis equal
rectangle('Position',[0 -14.9063  47 14.9063   ],'LineStyle','-','LineWidth',0.5)
rectangle('Position',[7 -8 6 6   ],'LineStyle','-','LineWidth',0.5,'EdgeColor','r')


rectangle('Position',[30 -13 10 3  ],'LineStyle','-','LineWidth',0.5,'EdgeColor','r')


%%% also for IP
load('Results_IP.mat')
clear zones sigma_zone
for i = 1:size(sigma,2)
    sigma_zone(:,i) = unique(sigma(:,i))' ;
    [~,~,zones(:,i)] = unique(sigma(:,i)) ;
end
