...
sigma0 = load('Results_DC.mat','sigma').sigma; % keep as S/m
phase = load('Results_IP.mat','sigma').sigma; % change iter # if needed
...
b_mean = 0.97;
c_mean = 2.29;
constant = 3; % account for S/m to mS/m in sigma0 and sigma2
...
for ii = 1:3
    subplot(3,1,ii)
    if ii==2,ylabel('$Y [\mathrm{m}]$','Interpreter','latex','FontSize',16), end
    plot_vtk_2D()
    ...
    if(ii==1), caxis ([min(k_mean),max(k_mean)]); end
    axis equal
    ylim([-3.52 0])
end
