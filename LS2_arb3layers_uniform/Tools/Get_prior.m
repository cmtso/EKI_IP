...
L_means2=zeros(dim,N_En); %%check the 3 hard coded here
L_per_x=zeros(Grid.N,N_En);
L_per_y=zeros(Grid.N,N_En);
L_per_x2=zeros(Grid.N,N_En);
L_per_y2=zeros(Grid.N,N_En);
RN=zeros(Grid.N,N_En);
RN2=zeros(Grid.N,N_En);
X = lhsdesign(N_En,dim);
X2 = lhsdesign(N_En,dim);


for i=1:N_En  % was using parfor
    RN(:,i)=randn(Grid.N,1);
    RN2(:,i)=randn(Grid.N,1);
    for idim=1:dim
        ...
        Y2=exp(Pr.level.len(idim).mean.lim(1))+(exp(Pr.level.len(idim).mean.lim(2))-exp(Pr.level.len(idim).mean.lim(1)))*X2(i,idim);
        L_means2(idim,i)= log(Y2);
    end
    if (Pr.level.len(idim).per.sigma>0)
        L_per_x(:,i)=grf2D(Grid, Pr.level.len(1).per,randn(Grid.N,1));
        L_per_y(:,i)=grf2D(Grid, Pr.level.len(2).per,randn(Grid.N,1));        
        L_per_x2(:,i)=grf2D(Grid, Pr.level.len(1).per,randn(Grid.N,1));
        L_per_y2(:,i)=grf2D(Grid, Pr.level.len(2).per,randn(Grid.N,1));        
    end
end
