...
L=[60,20]; %dimensions of the 2D domain where we wish to recover conductivity 
...
n_fields=4;  %number of fields 2 or 3 
...
data=get_R2_data('protocol.dat'); % protocol.dat for field problems, cR2_forward.dat for synthetic
...
% %%generate synthetic data
% %cd .. % 
noise=0.01; %% percentage of noise added to true data
% 
% 
%%change this routine to read output (i.e. voltages) from Andy's code
data=get_R2_data('protocol.dat',7); % protocol.dat for field problems, cR2_forward.dat for synthetic

%%get data from e4d for the simulation with the true conductivity
Data.data_noise_free=data;
noise_data1= noise*data; %5.0; % abs phase error  %noise*data; %get_R2_data('protocol.dat',9); %noise*data;
noise_data2= 2.0; %mrad % Andy specifies %abs(max(data))*1e-5;%(max(abs(data))-min(abs(data)))*1e-4;
Data.data=data+noise_data1.*randn(length(data),1)+noise_data2.*randn(length(data),1); %%add two components of noise to the data
Data.inv_sqrt_C=diag(1./sqrt(noise_data1.^2+noise_data2.^2)); %inverse of square root of measurement error covariance
save('Data_IP','Data')
...
hold on
pgon = polyshape([0 0 10 40 120 120 -150],...
    [0 -11 -9 -4 -4 0 0]);
plot(pgon, 'FaceColor', 'none','EdgeColor','r')
pgon = polyshape([10 30 50 120 120 40 10],...
    [-9 -11 -11 -11 -4 -4 -9]);
plot(pgon, 'FaceColor', 'none','EdgeColor','r')
xlim([0 47])
ylim([-15.33 0])
rectangle('Position',[0 -15.33 47 15.33],'LineStyle','-','LineWidth',0.5)
% ylim([-8.66 0])
% xlim([0 3.96])
caxis([0 2.5])
caxis([-20 -5])
...
vtk.scalar_list(end+1:end+numel(add_list)) = add_list; 
vtk.scalar_data = [vtk.scalar_data (sigma_mean) ...
    sigma_std' sigma_std'./(sigma_mean)] ;
add_list = {"mean phase (mrad)", ...
    "std(phase)", "CV(phase)"} ;
vtk.scalar_list(end+1:end+numel(add_list)) = add_list; 
%%%
