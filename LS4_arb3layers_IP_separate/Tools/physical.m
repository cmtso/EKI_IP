function [sigma, U1]=physical(Grid,R2_Grid,Pr,Un,data_type)
...
U_temp=grf';
U1 = interp2(Grid.X,Grid.Y,U_temp,R2_Grid.x,R2_Grid.y,'spline',0);
...
switch data_type
    case 'IP' % no log conversion
        ...
        sigma1=K{1}.*(U1<cut(1) & U2<cut(2));
        sigma2=K{2}.*(U1<cut(1) & U2>=cut(2));
        sigma3=K{3}.*(U1>=cut(1) & U2>=cut(2));
        sigma4=K{4}.*(U1>=cut(1) & U2<cut(2));
    ...
    case 'DC' % convert from log to normal space
        ...
        sigma1=exp(K{1}).*(U1<cut(1) & U2<cut(2));
        sigma2=exp(K{2}).*(U1<cut(1) & U2>=cut(2));
        sigma3=exp(K{3}).*(U1>=cut(1) & U2>=cut(2));
        sigma4=exp(K{4}).*(U1>=cut(1) & U2<cut(2));
        sigma=sigma1+sigma2+sigma3+sigma4;

        %         sigma1_temp=exp(K{1}).*(U_temp<cut(1) & U_temp2<cut(2));
        %         sigma2_temp=exp(K{2}).*(U_temp<cut(1) & U_temp2>=cut(2));
        %         sigma3_temp=exp(K{3}).*(U_temp>=cut(1) & U_temp2>=cut(2));
        %         sigma4_temp=exp(K{4}).*(U_temp>=cut(1) & U_temp2<cut(2));
        %         sigma_temp=sigma1_temp+sigma2_temp+sigma3_temp+sigma4_temp;
